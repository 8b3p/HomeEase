...
const ChoresList = ({ chores, choreAssignments, users, session }: props) => {
  const mine = useMemo(() => {
    return choreAssignments.length > 0
      ? choreAssignments.filter(chore => chore.userId === session.user?.id)
      : [];
  ...
  return (
    <Stack justifyContent='center' alignItems='start' width='100%' spacing={2}>
      <Stack width='100%' spacing={1} paddingBottom='2rem'>
          ...
          <Stack width={isMobile ? "100%" : "50%"} padding={2}>
            {mine.filter(c => c.status === "Pending").length === 0 ? (
              <Stack
                ...
                alignItems='center'
              >
                <Typography variant='h6'>No Pending Chores</Typography>
              </Stack>
            ) : (
              mine
                ...
                .map(choreAssignment => {
                  return (
                    <AssignmentItem
                      ...
                      session={session}
                      chore={chores.find(
                        chore => chore.id === choreAssignment.choreId
                      )}
                      firstname={
                        users.find(user => user.id === choreAssignment.userId)
                          ?.firstName || ""
                      }
                      lastname={
                        users.find(user => user.id === choreAssignment.userId)
          ...
          <Stack width={isMobile ? "100%" : "50%"} padding={2}>
            {mine.filter(
              ...
              <Stack
                ...
                alignItems='center'
              >
                <Typography variant='h6'>No Completed Chores</Typography>
              </Stack>
            ) : (
              <>
                {mine
