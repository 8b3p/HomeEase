...
const CreatePaymentForm = ({
  ...
  const [customAmountsError, setCustomAmountsError] = useState<{
    [key: string]: string;
  }>({});
  ...
  const handleCreatePayment = async () => {
    ...
    setIsLoading(true);
    const bodies: PaymentPostBody[] = payersId
      ...
      .map(
        payerId =>
          ({
            amount: parseFloat(
              (isCustom
                ? customAmounts[payerId]
                : (amount || 0) / payersId.length
              ).toFixed(2)
            ),
            payerId,
            description,
            ...
            <Stack spacing={2}>
              ...
              <TextField
                ...
                onChange={e => {
                  ...
                  paymentDateError && setPaymentDateError("");
                }}
              ...
              <FormControl error={payersIdError !== ""}>
                      ...
                      >
                        ...
                        >
                          ...
                          {payersId.indexOf(user.id!) > -1 && !isCustom ? (
                            ...
                            <Typography>{customAmounts[user.id!]}</Typography>
                          )}
                        </Stack>
                      </MenuItem>
                    );
                ...
                {payersIdError !== "" && (
                  <FormHelperText error>{payersIdError}</FormHelperText>
                )}
              </FormControl>
              <FormControlLabel
                control={
                        ...
                        setCustomAmounts(
                          payersId.reduce(
                            (acc, payerId) => ({
                              ...acc,
                              [payerId]: parseInt(
                                ((amount || 0) / payersId.length).toString()
                              ),
                            }),
                            {}
                          )
              ...
              {isCustom && (
                <Stack spacing={2}>
                  {payersId.map(payerId => {
                    ...
                    return (
                      <TextField
                        ...
                        hiddenLabel
                        defaultValue={((amount || 0) / payersId.length).toFixed(
                          2
                        )}
                        ...
                        error={
                          customAmountsError[payerId] !== undefined &&
                          customAmountsError[payerId] !== ""
                        }
                  ...
                  >
                    Total:{" "}
                    {Object.values(customAmounts).length
