...
const House = ({ baseUrl, session }: props) => {
  ...
  const [linkButtonContent, setContent] = useState<string | JSX.Element>('Invitation Link')
  ...
  return (
    <Stack height='100%' alignItems='center' justifyContent="start" paddingBottom={1} paddingTop={isSmallScreen ? 0 : 4} spacing={4}>
                ...
                <ListItemText
                  ...
                  secondary={
                    <Typography
                      ...
                      sx={{ display: 'block' }}
                      color="text.primary"
                    > {appVM.house?.invitationCode} </Typography>
                  }
                />
              </ListItem>
              <IconButton onClick={() => { appVM.house?.invitationCode && navigator.clipboard.writeText(appVM.house?.invitationCode) }}>
            ...
            <Grid container columns={21} spacing={1} width="100%">
              <Grid xs={10}>
                <Stack justifyContent="center" alignItems="center" height="100%" width="100%">
                  <Button onClick={copyLinkHandler} sx={{ cursor: 'pointer' }}>{linkButtonContent}</Button>
                </Stack>
              </Grid>
              <Grid xs={1}>
