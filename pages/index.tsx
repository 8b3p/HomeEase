...
import { Avatar, AvatarGroup, Button, Grid, Grow, Paper, Stack, Theme, Typography, useMediaQuery, useTheme, } from "@mui/material";
...
const Home = ({ house, session, balances }: props) => {
      ...
      <Grid container spacing={2} paddingTop={4} paddingBottom={4}>
        <Grid item xs={12}>
          <Stack justifyContent='center' alignItems='center'>
            <Paper sx={{ minWidth: "100%", boxShadow: theme.shadows[5], padding: theme.spacing(2), borderRadius: theme.shape.borderRadius, }} >
              <Stack spacing={2} direction={isSmallScreen ? "column" : "row"}
                justifyContent='space-between' alignItems={isSmallScreen ? "start" : "center"} >
                <Typography variant='h4' color={theme.palette.secondary.main} >
                  Member of &#39;{house.name}&#39;
                </Typography>
                <AvatarGroup max={4} sx={{ cursor: "pointer" }} onClick={() => router.push("profile")} >
                  {house.users.map(user => (
                    <Avatar {...stringAvatar(user.firstName + " " + user.lastName, themeVM.themeType)} key={user.id} />
                  ))}
                </AvatarGroup>
              </Stack>
            </Paper>
...
export const getServerSideProps: GetServerSideProps = async ctx => {
  ...
  if (session?.user?.houseId) {
    house = await prisma.house.findUnique({
      ...
      include: {
        users: true,
        choreAssignments: { include: { Chore: true, }, },
        payments: true,
      },
    });
    if (!house) {
      return { props: { session: session, house: null, }, };
    }
    house.payments = house.payments.filter(
      payment => (payment.payerId === session?.user?.id ||
        payment.recipientId === session?.user?.id) &&
        payment.status === "Pending"
    );
    house.choreAssignments = house.choreAssignments.filter(
      assignment => assignment.userId === session?.user?.id && assignment.status === "Pending"
    );
