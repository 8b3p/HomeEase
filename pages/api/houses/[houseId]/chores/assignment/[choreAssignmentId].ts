...
const handler = async (
  ...
  if (!isValidObjectId(choreAssignmentId)) return res.status(400).json({ message: 'Invalid choreAssignment id' })

  const choreAssignment = await prisma.choreAssignment.findUnique({
    where: { id: choreAssignmentId },
  })
  if (!choreAssignment) return res.status(404).json({ message: 'choreAssignment not found' })
